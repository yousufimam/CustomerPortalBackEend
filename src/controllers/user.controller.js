...
const fetchSuccess = (req, res) => {
  return res.status(200).json({ message: "Successfully successed" });
};

const fetchSpecificUser = (req, res) => {
  User.find({email: req.user.email}, function(err, found){
    const specificUser = found[0];
...
export { fetchSignIn, fetchSignUp, fetchSuccess, createUser, logInUser, logOutUser, fetchSpecificUser };
